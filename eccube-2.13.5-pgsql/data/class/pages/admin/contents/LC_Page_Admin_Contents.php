...
class LC_Page_Admin_Contents extends LC_Page_Admin_Ex
{
    ...
    public function action()
    {
        ...
        switch ($this->getMode()) {
            ...
            case 'pre_edit':
                ...
                list($news['news_start_year'],$news['news_start_month'],$news['news_start_day']) = $this->splitNewsDate($news['cast_news_start_date']); //追加
                list($news['news_end_year'],$news['news_end_month'],$news['news_end_day']) = $this->splitNewsDate($news['cast_news_end_date']); //追加
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam('開始日付(年)', 'news_start_year', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
        $objFormParam->addParam('開始日付(月)', 'news_start_month', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
        $objFormParam->addParam('開始日付(日)', 'news_start_day', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
        $objFormParam->addParam('終了日付(年)', 'news_end_year', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
        $objFormParam->addParam('終了日付(月)', 'news_end_month', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
        $objFormParam->addParam('終了日付(日)', 'news_end_day', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK')); //追加
    }
    ...
    public function doRegist($news_id, $sqlval, SC_Helper_News_Ex $objNews)
    {
        ...
        $sqlval['news_start_date'] = $this->getStartDate($sqlval); //追加
        unset($sqlval['news_start_year'], $sqlval['news_start_month'], $sqlval['news_start_day']); //追加
        $sqlval['news_end_date'] = $this->getEndDate($sqlval); //追加
        unset($sqlval['news_end_year'], $sqlval['news_end_month'], $sqlval['news_end_day']); //追加
    ...
    {
        $startDate = $arrPost['news_start_year'] .'/'. $arrPost['news_start_month'] .'/'. $arrPost['news_start_day'];
    ...
    {
        $endDate = $arrPost['news_end_year'] .'/'. $arrPost['news_end_month'] .'/'. $arrPost['news_end_day'];
